...
frappe.ui.form.on('Item', {
  refresh: function (frm) {
    // Keep item_code non-editable
    frm.set_df_property('item_code', 'read_only', 1);

    // Clear any pending preview debounce on refresh
    if (frm._preview_timeout) {
      clearTimeout(frm._preview_timeout);
      frm._preview_timeout = null;
    }

    // Show preview if: new doc + has item_group + no item_code
  ...
  item_group: function (frm) {
    // If no item_group, clear preview
